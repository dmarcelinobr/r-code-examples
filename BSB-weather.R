...
read.table() %>% data.frame %>% tbl_df -> BSB

# rename variables
names(BSB) <- c("Month", "Day", "Year", "Temp")
 # create dataframe that represents 1995-2013 historical data

# there is some overlaping
dim(BSB)

BSB <- BSB %>% filter(Month!=2 | Day!=29) # corrects for Feb

# function to convert Fahrenheit to Celsius

fahrenheit.to.celsius <-function (F, round = 1)
  {
    C <- (5/9) * (F - 32)
    C <- round(C, digits = round)
    return(C)
}

# function to convert Celsius to Fahrenheit
celsius.to.fahrenheit <- function (C, round = 1){
    F <- (9/5) * C + 32
    F <- round(F, digits = round)
    return(F)
}

BSB %>% mutate(TempC = fahrenheit.to.celsius(Temp)) -> BSB


# Convert Fa to Celcius
BSB
Past <- BSB %>%
group_by(Year, Month) %>%
arrange(Day) %>%
...
ungroup() %>%
filter(Temp != -99 & Year == 2015)  # filter out missing data & select current year data
...
# create y-axis variable
 a <- dgr_fmt(seq(0,50, by=5))
# create a small dataframe to represent legend symbol for 2015 Temperature

legend_data <- data.frame(x=seq(175,182),y=rnorm(8,15,2)) #Creating the Visual Graphic
...
breaks = c(15,45,75,105,135,165,195,228,258,288,320,350), labels = c("January", "February", "March", "April","May", "June", "July", "August", "September","October", "November", "December"))
print(p)
...
p <- p +
ggtitle("Brasilia's Weather in 2015") +
...
annotate("text", x = 162, y = 14.75, label = "2014 TEMPERATURE", size=2, colour="gray30") + annotate("text", x = 193, y = 25, label = "RECORD HIGH", size=2, colour="gray30") +
